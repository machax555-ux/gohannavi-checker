...

{
  ...
  "product_category": "単一の基本大分類カテゴリ名（絶対ルール参照）",
  "summary": "全体の判定サマリー（100文字以内）",
  "blog_keyword": "ブログ・EC検索用単語（例：無添加せんべい）"
}

【product_categoryおよびblog_keywordの絶対厳守ルール】
・修飾語（ごま、のり、黒糖、辛口、無塩、淡口、濃口、有機 など）を付加した複合語（例: 「ごませんべい」「のりせんべい」「濃口しょうゆ」「黒糖お菓子」「ごま菓子」等）の出力は【厳禁】とします。
・必ずデータベースの主要キーである単一の大分類基本カテゴリ名（例: 「せんべい」「ポテトチップス」「クッキー」「チョコレート」「しょうゆ」「めんつゆ」「ポン酢」「ドレッシング」「ウインナー」「ハム」「だし」「味噌」「かまぼこ」等）のみを出力してください。
・パッケージの名称が「菓子」「食品」等と抽象的な場合でも、原材料（ごま・米 ➔ せんべい / 馬鈴薯 ➔ ポテトチップス / 小麦粉・砂糖 ➔ クッキー など）から基本大分類カテゴリ名を推測して単一単語で出力してください。
