...
export default function HomePage() {
  ...
  return (
    <main className="h-full max-h-full flex flex-col justify-between overflow-hidden select-none">
      ...
      <header className="flex flex-col gap-1 shrink-0">
        <div className="flex items-center justify-between">
          <a
            ...
            className="bg-white px-2 py-0.5 md:px-3 md:py-1 swiss-border inline-flex items-center hover:opacity-90 transition-opacity"
          >
            <img
              src="/gohannavi-logo.png"
              alt="ごはんなび"
              className="h-5 sm:h-5.5 md:h-7 w-auto object-contain"
            />
          </a>
        ...
        <div className="flex flex-col gap-0.5 mt-0.5 md:flex-row md:items-end md:justify-between md:mt-2">
          <div>
            <h1 className="font-display font-black text-3xl sm:text-4xl md:text-5xl leading-none tracking-tight text-[#111111]">
              FOOD<br className="md:hidden" /> <span className="hidden md:inline"></span>CHECKER
            </h1>
            <p className="text-[11px] sm:text-xs md:text-sm font-black text-[#111111] tracking-wider mt-0.5 md:mt-1">
              食品添加物を瞬時に判定するアプリ
            </p>
          </div>
        </div>

        <div className="w-full h-[2px] md:h-[3px] bg-[#111111] mt-1 md:mt-2" />
      </header>

      {/* 2. Main Hero Cards Container */}
      {/* MOBILE DISPLAY (767px & below) - 100% EXPLICITLY PRESERVED */}
      ...
      {/* PC DISPLAY (768px & above) - 2-COLUMN GRID LAYOUT */}
      <div className="hidden md:grid grid-cols-2 gap-6 flex-1 my-4 min-h-0 overflow-y-auto">
        {/* Left Column: 01. DATABASE + 03. HOW IT WORKS */}
        <div className="flex flex-col gap-5 justify-between">
          {/* 01. DATABASE Card */}
          <Link
            href="/search"
            className="swiss-card-dark p-6 flex flex-col justify-between gap-4 group hover:opacity-95 transition-opacity cursor-pointer block h-full"
          >
            <div className="flex items-center justify-between border-b border-white/20 pb-2">
              <span className="font-display font-black text-sm text-[#F5CE42] tracking-widest">
                01. DATABASE
              </span>
              <span className="text-xs text-white/60 font-bold">無添加商品データベース</span>
            </div>

            <div>
              <h2 className="font-extrabold text-xl text-white mb-1">
                無添加食品データベース
              </h2>
            ...
            <div className="w-full py-4 px-6 bg-[#EF4444] text-white swiss-border font-black text-base flex items-center justify-center gap-3 group-hover:bg-[#DC2626] transition-colors shadow-md">
          ...
          <section className="swiss-card-white p-6 flex flex-col justify-between gap-4 h-full">
            <h3 className="font-display font-black text-sm text-[#111111] tracking-widest border-b-2 border-black pb-2 flex items-center justify-between">
              <span>03. HOW IT WORKS</span>
              <span className="text-xs">アプリの使い方</span>
            </h3>

            <div className="grid grid-cols-2 gap-4 my-auto">
              <div className="flex flex-col gap-2 p-3 bg-[#FAF9F5] swiss-border-sm">
                <div className="flex items-center gap-2">
                  <span className="font-display font-black text-xs bg-[#111111] text-[#F5CE42] px-2 py-0.5 shrink-0">
                    STEP 01
                  </span>
                </div>
                <h4 className="font-extrabold text-sm text-[#111111]">原材料名を撮影・入力</h4>
                <p className="text-xs text-[#555555] font-medium leading-relaxed">
                  商品の裏面ラベルをスマホのカメラで撮影、またはテキストを直接入力します。
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2 p-3 bg-[#FAF9F5] swiss-border-sm">
                ...
                <h4 className="font-extrabold text-sm text-[#111111]">AIが添加物を瞬時判定</h4>
                <p className="text-xs text-[#555555] font-medium leading-relaxed">
                  高精度AIが気になる添加物を抽出し、リスク評価と安心の無添加代替案をご提案。
                </p>
              </div>
            </div>
          </section>
        ...
        <div className="flex flex-col gap-5 justify-between">
          {/* 02. SCANNER Card */}
          <div className="swiss-card-dark p-6 flex flex-col justify-between gap-4 h-full relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-white/20 pb-2">
              <span className="font-display font-black text-sm text-[#F5CE42] tracking-widest">
                02. SCANNER
              </span>
              <span className="text-xs text-[#F5CE42] font-black">メイン解析機能</span>
            </div>

            <div>
              <h2 className="font-black text-2xl text-white mb-1 leading-snug">
                原材料名を撮影・入力する
              </h2>
              <p className="text-xs text-white/80 font-medium">
                写真をアップロードするか、原材料テキストを貼り付けるだけで、危険な食品添加物を自動検出します。
              </p>
            </div>

            {mounted && !usable ? (
              <div className="w-full py-4 bg-[#333333] text-white/50 swiss-border text-center font-extrabold text-sm flex items-center justify-center gap-2 cursor-not-allowed">
              ...
              <Link
                href="/scan"
                className="w-full py-4 bg-[#EF4444] hover:bg-[#DC2626] text-white swiss-border swiss-shadow font-black text-lg flex items-center justify-center gap-3 transition-transform active:translate-x-0.5 active:translate-y-0.5 group"
              >
          ...
          {/* 04. INSTALL Card */}
          {mounted && !isStandalone && (
            <div className="swiss-card-dark p-6 flex flex-col justify-between gap-3 h-auto group cursor-pointer block">
              <div className="flex items-center justify-between border-b border-white/20 pb-1.5">
                ...
                <span className="text-xs text-white/60 font-bold">PWAアプリ化</span>
              </div>
              <p className="text-xs text-white/80 font-medium">
                Webブラウザだけでなく、スマホやPCのホーム画面に追加してアプリとしてサクサク起動できます。
              </p>
              <button
                onClick={handleInstallClick}
                className="w-full py-3.5 px-4 bg-[#F5CE42] hover:bg-[#E5BE32] text-[#111111] swiss-border font-black text-base flex items-center justify-center gap-2 transition-colors cursor-pointer group"
              >
      ...
      {/* 4. Footer */}
      <footer className="pt-1.5 md:pt-3 border-t-2 border-black flex items-center justify-between text-xs shrink-0">
        <a
          href="https://gohannavi.com"
          target="_blank"
          rel="noopener noreferrer"
